      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {packages.map((pkg, i) => (
          <motion.div
            ...
            className={`relative bg-card rounded-2xl p-8 shadow-card hover:shadow-card-hover transition-all duration-500 border hover:-translate-y-1 ${
              pkg.featured ? "border-accent/30 ring-2 ring-accent/10 scale-[1.02]" : "border-border/50"
            }`}
          >
            {pkg.featured && (
            ...
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${
              ...
              <pkg.icon className={`w-7 h-7 ${pkg.featured ? "text-accent" : "text-primary"}`} strokeWidth={1.5} />
            </div>
            ...
            <div className="mb-6">
              ...
              <span className="text-4xl font-extrabold text-foreground ml-1.5 tracking-tight">{pkg.price}</span>
